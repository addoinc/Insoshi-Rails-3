module ActivitiesHelper
  ...
  def feed_message(activity, recent = false)
    ...
    when "BlogPost"
      ...
      if recent
        raw("new blog post  #{post_link(blog, post)}")
      else
        raw( "#{person_link_with_image(person)} posted
    ...
    when "Comment"
      ...
      when "BlogPost"
        ...
        blog = post.blog
        if recent
          raw("made a comment to #{someones(blog.person, person)} blog post
            #{post_link(blog, post)}")
        else
          raw("#{person_link_with_image(person)} made a comment to
            #{someones(blog.person, person)} blog post
            #{post_link(blog, post)}")
        end
      when "Person"
        if recent
          raw("commented on #{wall(activity)}")
        else
          raw("#{person_link_with_image(activity.item.commenter)}
    ...
    when "Event"
      ...
      raw("#{person_link_with_image(commenter)} commented on 
        #{someones(event.person, commenter)} event: 
        #{event_link(event.title, event)}.")
    when "Connection"
      if activity.item.contact.admin?
        if recent
          raw("joined the system")
        else
          raw("#{person_link_with_image(activity.item.person)}
            has joined the system")
        end
      else
        if recent
          raw("connected with #{person_link_with_image(activity.item.contact)}")
        else
          raw("#{person_link_with_image(activity.item.person)} and
    ...
    when "ForumPost"
      post = activity.item
      if recent
        raw("new post to forum topic #{topic_link(post.topic)}")
      else
        raw("#{person_link_with_image(person)} made a post to forum topic
          #{topic_link(post.topic)}")
      end
    when "Topic"
      if recent
        raw("new discussion topic #{topic_link(activity.item)}")
      else
        raw("#{person_link_with_image(person)} created the new discussion topic
    ...
    when "Person"
      if recent
        ...
        raw("#{person_link_with_image(person)}'s description changed")
      end
    when "Gallery"
      if recent
        raw("new gallery #{gallery_link(activity.item)}")
      else
        raw("#{person_link_with_image(person)} added a new gallery #{gallery_link(activity.item)}")
      end
    when "Photo"
      if recent
        raw("added new #{photo_link(activity.item)}
          #{to_gallery_link(activity.item.gallery)}")
      else
        raw("#{person_link_with_image(person)} added a new
          #{photo_link(activity.item)}
          #{to_gallery_link(activity.item.gallery)}")
      end
    when "Event"
      event = activity.item
      raw("#{person_link_with_image(person)} has created a new event:
        #{event_link(event.title, event)}.")
    when "EventAttendee"
      event = activity.item.event
      raw("#{person_link_with_image(person)} is attending
  ...
  def minifeed_message(activity)
    ...
    when "Comment"
      ...
      when "BlogPost"
        post = activity.item.commentable
        blog = post.blog
        raw("#{person_link(person)} made a comment on
          #{someones(blog.person, person)} 
          #{post_link("blog post", post.blog, post)}")
      when "Person"
        raw("#{person_link(activity.item.commenter)} commented on 
          #{wall(activity)}.")
      when "Event"
        event = activity.item.commentable
        raw("#{person_link(activity.item.commenter)} commented on 
    ...
    when "Connection"
      if activity.item.contact.admin?
        raw("#{person_link(person)} has joined the system")
      else
        raw("#{person_link(person)} and
    ...
    when "Topic"
      raw("#{person_link(person)} created a 
        #{topic_link("new discussion topic", activity.item)}")
    when "Person"
      raw("#{person_link(person)}'s description changed")
    when "Gallery"
      raw("#{person_link(person)} added a new gallery
        #{gallery_link(activity.item)}")
    when "Photo"
      raw("#{person_link(person)} added new
        #{photo_link(activity.item)} #{to_gallery_link(activity.item.gallery)}")
      raw("#{person_link(person)}'s description has changed.")
    when "Event"
      raw("#{person_link(person)}'s has created a new
        #{event_link("event", activity.item)}.")
    when "EventAttendee"
      event = activity.item.event
      raw("#{person_link(person)} is attending
